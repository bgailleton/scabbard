...
# tau_c = 4
k_z = tau_c/(0.4 * (rho_sediment - rho_water) * gravity)
# k_h = 
k_h = 1
# K = 1e-3
# NOTE THAT RHO SED AND RHO WATER ARE INCLUDED IN k_z and k_h
...
@ti.func
def Psi(i,j) -> ti.f32:
	return (k_h * rho_water * hw[i,j] * Zw(i,j) + k_z * (rho_sediment - rho_water) * Z[i,j])/(k_h * hw[i,j] + k_z)
...
def compute_QwQs():
	for i,j in Z:
		...
		graviterm = k_z * (rho_sediment - rho_water) * ti.math.sqrt(SSgradery*SSgradery + SSgraderx*SSgraderx)
		waterm = k_h * hw[i,j] * gradSw * rho_water
		graderr = graviterm + waterm
		gradSPsi = ti.math.sqrt(SSPsiy*SSPsiy + SSPsix*SSPsix)

		monitorer[i,j] = graviterm - tau_c

		QwC[i,j] = dx/manning * ti.math.pow(hw[i,j], 5./3) * sumSPsi/ti.math.sqrt(gradSPsi)

		edot = K * ti.math.pow( ti.math.max(gravity * graderr - tau_c, 0.),1.5);
		...
		QsC[i,j] = (QsA[i,j] + ti.math.pow(dx,2) * (edot))/(1 + depcof);
