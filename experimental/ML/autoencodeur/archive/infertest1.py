...
model.load_state_dict(torch.load('autoencoder_bobo.pth', weights_only = True))

# Set the model to evaluation mode if you're done training
model.eval()

temp = torch.FloatTensor(Ztest.ravel()).view(1, -1).to(device)
print(temp)
with torch.no_grad():
...
while True:
	...
	im0.set_data(Ztest)
	im1.set_data(Z_decomp.view(ny,nx).cpu())
