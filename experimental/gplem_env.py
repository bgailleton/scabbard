...
env = scb.env_from_DEM('/home/bgailleton/code/FastFlood2.0/FastFlood2_Boris/graphflood/paper_scripts/data/green_river_1.tif')
nx,ny = env.grid.nx, env.grid.ny
dx,dy = env.grid.dy, env.grid.dx
...
Z = env.grid._Z.astype(np.float32)
# Z[0,:] += 100
# Z = Z.ravel()
# print(Z.shape, nx*ny)
# quit()

# plt.imshow(env.grid._Z.reshape(env.grid.rshp))
# plt.show()
# quit()
...
add_Qwin_global = mod.get_function("add_Qwin_global")
...
# drv.memcpy_dtoh(BC, BC_gpu)
# plt.imshow(BC.reshape(ny,nx))
# plt.show()
# quit()
...
im = ax.imshow(hw.reshape(ny,nx), cmap = "Blues", vmin = 0, vmax = 1)
plt.colorbar(im)
...
for step in range(N_iterations):
    ...
    add_Qwin_global(QwA_gpu, np.float32(50 * 1e-3/3600), block = block, grid = grid)
